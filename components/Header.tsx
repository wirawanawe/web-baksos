...
export default function Header() {
  ...
  const getRoleRoutes = () => {
    const roleRoutes: Record<string, { label: string; path: string }[]> = {
      ...
      dokter: [
        { label: 'Pemeriksaan', path: '/dokter' },
      ],
      farmasi: [
        { label: 'Farmasi', path: '/farmasi' },
      ],
    };
  ...
  const getCurrentPageLabel = () => {
    ...
    return currentRoute?.label || 'Menu';
  };
        ...
        {routes.length > 0 && (
          <div className={styles.dropdownContainer} ref={menuRef}>
            <button
              ...
              className={`${styles.dropdownButton} ${showMenu ? styles.active : ''}`}
            >
              <svg
              ...
              <div className={styles.dropdownMenu}>
                {routes.map((route) => (
                  <button
                    ...
                    className={`${styles.dropdownItem} ${pathname === route.path ? styles.dropdownItemActive : ''}`}
                  >
                    {route.label}
                  </button>
                ))}
